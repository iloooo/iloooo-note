...
> iloooo:[add:表格和参考](https://www.cnblogs.com/holly8/p/11425080.html)
>
> RequestMapping是一个用来处理请求地址映射的注解，可用于**类或方法**上
>
> | 参数     | 默认值 | 描述                                                         |
> | -------- | ------ | ------------------------------------------------------------ |
> | value    |        | 指定请求的实际地址，指定的地址可以是URI Template 模式（含有某变量的一类值、含正则表达式的一类值） |
> | consumes |        | request内容类型（Content-Type），例如application/json        |
> | produces |        | response的内容类型，仅当request请求头中的(Accept)类型中包含该指定类型才返回； |
